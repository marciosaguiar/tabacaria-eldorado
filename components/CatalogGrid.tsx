...
// ─── Lightbox ────────────────────────────────────────────────────────────────
function Lightbox({ product, channel, whatsapp, onClose }: {
  product: Product
  ...
  return (
    ...
    >
      ...
      {/* Imagem */}
      <div className="flex-1 relative mx-4 rounded-sm overflow-hidden" onClick={e => e.stopPropagation()}>
        {product.imagem ? (
          <Image src={product.imagem} alt={product.nome} fill
            style={{ objectFit: 'contain' }} sizes="100vw" priority />
        ) : (
      ...
      <div className="flex-shrink-0 px-4 pt-3 pb-5" onClick={e => e.stopPropagation()}>
        {product.descricao && (
          <p className="font-inter text-sm mb-3 leading-relaxed" style={{ color: 'rgba(255,255,255,0.6)' }}>
            {product.descricao}
          </p>
        )}
        <div className="flex items-center justify-between gap-3">
          {fmt(price) ? (
            <p className="font-inter font-bold text-2xl sm:text-3xl tracking-tight" style={{ color: 'var(--gold)' }}>
              {fmt(price)}
            </p>
          ) : <span />}
          {waLink && (
            <a href={waLink} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-5 py-2.5 rounded-full font-inter font-semibold text-sm transition-all hover:scale-105 active:scale-95"
...
function ProductCard({ product, channel, whatsapp, onImageClick }: { product: Product; channel: 'varejo' | 'atacado'; whatsapp: string; onImageClick: () => void }) {
  const price = channel === 'varejo' ? product.precoVarejo : product.precoAtacado
  ...
  return (
    <div className="product-card group relative rounded-sm overflow-hidden flex flex-col"
      ...
      <div className="relative aspect-square overflow-hidden cursor-zoom-in" style={{ backgroundColor: 'var(--bg-hover)' }}
...
export default function CatalogGrid({ initialProducts, categorias, channel, whatsapp = '' }: Props) {
  ...
  return (
    <>
      {/* Lightbox */}
      {lightbox && (
        <Lightbox product={lightbox} channel={channel} whatsapp={whatsapp} onClose={closeLightbox} />
      )}

      {/* Botão Voltar */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4">
        <Link href="/"
          className="inline-flex items-center gap-1.5 text-xs font-inter transition-opacity hover:opacity-70"
        ...
        {visible.length === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5">
              {visible.map(p => (
                <ProductCard key={p.id} product={p} channel={channel} whatsapp={whatsapp}
