...
export default function VarejoPage() {
  ...
  const visible = products.filter(p => p.visivelVarejo && (filterCat === 'todas' || p.categoria === filterCat))
  const cats = ['todas', ...categorias.filter(c => products.some(p => p.visivelVarejo && p.categoria === c))]
  ...
  return (
    <div className="min-h-screen flex flex-col bg-dark">
      ...
      <Footer />
      <AdminProductFAB channel="varejo" categorias={categorias} onAdded={p => setProducts(prev => [...prev, p])} />
    </div>
  )
}
