...
export default function AtacadoPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setCategorias(settings.categorias || [])
      setLoading(false)
    }).catch(() => setLoading(false))
  }, [])

  const visible = products.filter(p => p.visivelAtacado && (filterCat === 'todas' || p.categoria === filterCat))
  const cats = ['todas', ...categorias.filter(c => products.some(p => p.visivelAtacado && p.categoria === c))]
  ...
  return (
    <div className="min-h-screen flex flex-col bg-dark">
      ...
      <div className="relative overflow-hidden" style={{ borderBottom: '1px solid rgba(var(--gold-rgb),0.12)' }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16 text-center">
          ...
          <h1 className="font-playfair text-3xl sm:text-4xl md:text-5xl mb-4 leading-tight" style={{ color: 'var(--text-primary)' }}>
            Catálogo de <span className="text-gold-shine">Atacado</span>
          </h1>
      ...
      <Footer />
      <AdminProductFAB channel="atacado" categorias={categorias} onAdded={p => setProducts(prev => [...prev, p])} />
    </div>
  )
}
